...
def main():
    ...
    for data in [("/home/abaur/data/yolov5_dvs_c80_seq/data.yaml", "models/dvs_F.yaml"), ("/home/abaur/data/yolov5_rgb_c50_seq/data.yaml", "models/yolov5_dvs.yaml")]:
      for method in ["l1_unstructured", "ln_structured"]:
        for pa in [0.1, 0.5, 0.75, 0.9, 0.95]:
          command = f"python train.py --img 600 --batch 20 --epochs 50 --data {data[0]} --weights '' " \
                    f"--cache --cfg {data[1]} --wandblog True --project dvs_img_finally_pruning --name {method} --prune t --prune_amount {pa}"
