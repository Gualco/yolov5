...
def test(data,
         ...
         plots=True,
         log_imgs=0,
         time_seq_len=5):  # number of logged images
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            # the 2 lines beloaw are the same
            # img.unsqueeze(0).repeat(5, 1, 1, 1, 1)
            # torch.reshape(torch.cat(5*[img]), (5, nb, channels, height, width))
            #forward img time scale:
            # adds a Dimenson and repeats all others in it
            #  logger.debug(f'test input: {img.size()} {img.size(1)}')
            img_repeated = img.unsqueeze(0).repeat(time_seq_len, 1, 1, 1, 1).to(device)
            ...
            # img_repeated = time_image_seq.to(device=device).half() if half else time_image_seq.float()
